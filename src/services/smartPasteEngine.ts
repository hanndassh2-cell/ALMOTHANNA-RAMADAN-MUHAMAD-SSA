...
export async function cleanAndConvertHtml(
  ...
  if (!html && !files?.length) {
    ...
    const formattedPlain = formattedText ? `<p>${formattedText.replace(/\n/g, "<br>")}</p>` : "";
  ...
  for (let i = 0; i < parsedParagraphs.length; i++) {
    ...
    if (isRealList) {
      ...
      if (curr.parsed && curr.parsed.isList && curr.parsed.marker) {
        ...
        cleanHtml = cleanHtml.replace(/^\s*(\d+|[a-zA-ZIVXLCDMivxlcdm]+)[\.\)]\s*/, "");
        cleanHtml = cleanHtml.replace(/^\s*[·•o§\-*◆◇■□★☆✓✔➤🔹💡📌]\s*/, "");
        cleanHtml = cleanHtml.replace(/<span[^>]*style="[^"]*mso-list:Ignore[^"]*"[^>]*>.*?<\/span>/gi, "");
        cleanHtml = cleanHtml.replace(/<span[^>]*font-family:\s*Symbol[^>]*>.*?<\/span>/gi, "");
      }
