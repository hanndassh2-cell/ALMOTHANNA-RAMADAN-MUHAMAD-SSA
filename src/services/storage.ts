...
class StorageService {
  ...
  deleteSubject(id: string) {
    ...
    setStoredData(KEYS.SUBJECTS, list);
    this.logAction("المستخدم", "حذف مادة دراسية", "المواد", id);
  }
  ...
  deleteUnit(id: string) {
    ...
    setStoredData(KEYS.UNITS, list);
    this.logAction("المستخدم", "حذف وحدة دراسية", "الوحدات", id);
  }
  ...
  checkQuestionUsage(questionId: string): QuestionUsageDetails {
    ...
    // 1. Check Active Lesson Cards (KEYS.LESSON_CARDS) - Direct card question check
    const allCards = getStoredData<Record<string, any[]>>(KEYS.LESSON_CARDS, {});
  ...
  getPrintTemplates(): PrintTemplate[] {
    return getStoredData(KEYS.PRINT_TEMPLATES, initialPrintTemplates);
  }
  ...
  exportFullDatabaseJson(): string {
    ...
    const dbDump = {
      ...
      examTemplates: this.getExamTemplates(),
      exams: this.getExams(),
