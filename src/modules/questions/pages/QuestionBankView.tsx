...
export const QuestionBankView: React.FC<QuestionBankViewProps> = ({
  ...
  const allowedQuestions = useMemo(() => filterAllowedItemsBySubject(currentUser, questions), [currentUser, questions]);
  ...
  const handleBulkImport = async () => {
    ...
    const availableImportUnits = (units || []).filter(
    ...
    const availableImportLessons = (lessons || []).filter(
  ...
  const handleFinalizeImportSelected = () => {
    ...
    selectedValid.forEach((q) => {
      onSaveQuestion(q);
    });

    const remainingQuestions = [...selectedInvalid, ...unselected];

    if (remainingQuestions.length > 0) {
      alert(`✨ تم إرسال (${selectedValid.length}) سؤال بنجاح! يرجى إكمال مراجعة الأسئلة المتبقية.`);
      setExtractedQuestionsForPreview(remainingQuestions);
      setSelectedImportQuestionIds(new Set(selectedInvalid.map(q => q.id)));
    } else {
      alert(`✨ تم رفد بنك الأسئلة بـ ${selectedValid.length} سؤال بنجاح!`);
  ...
  const handleFinalizeImportAll = () => {
    ...
    if (invalidQuestions.length > 0) {
      alert(`✨ تم إرسال (${validQuestions.length}) سؤال بنجاح! يرجى إكمال الأسئلة المتبقية التي تحتاج إلى مراجعة.`);
      setExtractedQuestionsForPreview(invalidQuestions);
      setSelectedImportQuestionIds(new Set(invalidQuestions.map(q => q.id)));
    } else {
      alert(`✨ تم رفد بنك الأسئلة بكافة الأسئلة (${validQuestions.length}) بنجاح!`);
      setExtractedQuestionsForPreview(null);
      setSelectedImportQuestionIds(new Set());
      setRawImportText("");
      setActiveTab("questions");
      ...
      {activeTab === "questions" && (
        <div className="space-y-3">
          ...
          <div className="bg-white dark:bg-slate-900 p-3 sm:p-3.5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-2xs space-y-3 text-xs sm:text-sm">
                  ...
                  >
                    ...
                    {lessons
                      .filter((l) => {
                        ...
                        if (selectedSubjectId !== "all") {
                          return units.some((u) => u && u.id === l.unitId && u.subjectId === selectedSubjectId);
                        }
                        return true;
                      })
                      .map((l) => (
            ...
            {/* Sub-row: Clear Filters & Sort Option */}
          ...
          {showExecutionCenterForImport && (
            <AIExecutionCenterModal
              ...
              taskPayload={{
                ...
                subjectName: subjects.find((s) => s.id === importSubjectId)?.name || "عام",
              }}
              ...
              onSuccessResult={(res) => {
                if (res?.parsedQuestions && res.parsedQuestions.length > 0) {
                  // Process questions
                  const availableImportUnits = units.filter((u) => u && u.subjectId === importSubjectId);
                  const availableImportLessons = lessons.filter((l) => l && l.unitId === importUnitId);
