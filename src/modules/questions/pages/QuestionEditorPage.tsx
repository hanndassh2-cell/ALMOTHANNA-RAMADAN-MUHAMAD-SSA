...
export const QuestionEditorPage: React.FC<QuestionEditorPageProps> = ({
  ...
  const [formSubjectId, setFormSubjectId] = useState<string>(
    editingQuestion ? editingQuestion.subjectId : initialSubjectId || subjects[0]?.id || ""
  );
  ...
  const availableUnits = (units || []).filter((u) => u && u.subjectId === formSubjectId);
  const availableLessons = (lessons || []).filter((l) => l && l.unitId === formUnitId);
