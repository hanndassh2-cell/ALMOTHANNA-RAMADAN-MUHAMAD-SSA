...
export const ExamsLibraryView: React.FC<ExamsLibraryViewProps> = ({
  ...
  const allowedLibraryExams = useMemo(() => filterAllowedItemsBySubject(currentUser, libraryExams), [currentUser, libraryExams]);
  ...
  const previewExamItems = useMemo(() => {
    ...
    return generateExamPrintItems({
      template: previewExam.printTemplate || activeTemplate,
  ...
  }, [previewExam, showAnswerKey, activeTemplate]);
    ...
    >
                ...
                <div className="p-3 bg-slate-50 dark:bg-slate-800/80 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-1.5 flex-wrap">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <button
                      ...
                      className="px-2.5 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-[11px] font-bold transition flex items-center gap-1 shadow-2xs cursor-pointer"
                    ...
                    <button
                      ...
                      className="px-2.5 py-1.5 rounded-lg bg-amber-500 hover:bg-amber-600 text-white text-[11px] font-bold transition flex items-center gap-1 shadow-2xs cursor-pointer"
                      title="تعديل نموذج الاختبار في محرك التوليد"
                    >
                  ...
                  <button
                    onClick={() => setDeleteConfirmExamId(examId)}
                    ...
                    title="حذف نموذج الاختبار من المكتبة"
                  >
      ...
      {previewExam && (
        <PrintPreviewModal
          ...
          title={`نموذج اختباري - ${previewExam.title}`}
          extraToolbarContent={
            <div className="flex items-center gap-2">
              <button
          ...
          <PaginatedA4Preview
            template={{
              ...
              headerContent: {
                ...
                subjectName: subjects.find(s => s.id === (previewExam as any).subjectId)?.name || "نموذج اختبار معتمد",
              },
            }}
            title={previewExam.title}
            hierarchyText={(() => {
              ...
              const unitList = units.filter(u => previewExam.scope?.unitIds?.includes(u.id));
              if (unitList.length === 1) {
                let text = `${subName} - ${unitList[0].title}`;
                if (previewExam.scope?.lessonIds && previewExam.scope.lessonIds.length === 1) {
                  const les = lessons.find(l => l.id === previewExam.scope?.lessonIds?.[0]);
