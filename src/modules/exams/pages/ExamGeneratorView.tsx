...
export const ExamGeneratorView: React.FC<ExamGeneratorViewProps> = ({
  ...
  // Ensure selectedSubjectId is always set to a valid subject
  useEffect(() => {
  ...
  }, [subjects, selectedSubjectId, initialExamToEdit]);
  ...
  useEffect(() => {
    if (!initialExamToEdit) {
      ...
      if (subjects && subjects.length > 0 && !selectedSubjectId) {
        ...
        if (subjects[0].totalMarks) {
          setTotalMarks(subjects[0].totalMarks);
        }
      }
      setCustomSections([
        {
  ...
  useEffect(() => {
    ...
    setExamTitle(base);
  }, [selectedSubjectId, selectedUnitIds, selectedLessonIds, subjects, currentSubjectUnits, currentUnitLessons, initialExamToEdit, isAllUnitsSelected, isAllLessonsSelected]);
  ...
  }, [activeTemplate, currentSubject?.name, showInstitutionLogo]);
  ...
  }, [selectedSubjectId, subjects, initialExamToEdit]);
  ...
  const generateAutoMultiDrafts = async () => {
    ...
    try {
      ...
      buckets.forEach((bucketQs, idx) => {
        ...
        // Group bucketQs by type to form sections
        ...
        const snapshots = examLibraryService.createQuestionSnapshots(bucketQs, marksMap, {});
  ...
  const handleApproveMultiDrafts = () => {
    ...
    multiExamDrafts.forEach(draft => {
      if (draft.libraryDoc) {
        examLibraryService.saveExamToLibrary(draft.libraryDoc as any);
      }
      onSaveExam(draft);
    });
  ...
  return (
    <>
      ...
      <SplitWorkspaceLayout
        header={
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-5 border-b border-slate-200 dark:border-slate-800 shadow-sm z-30">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => setShowExecutionCenterModal(true)}
                ...
                title="مركز التنفيذ العالمي للذكاء الاصطناعي"
              >
              ...
              <button
                ...
                className="px-4 py-2 rounded-xl bg-slate-800 hover:bg-slate-900 text-white font-bold text-xs shadow-sm transition flex items-center gap-2 disabled:opacity-50"
              >
                  ...
                  <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl shadow-sm space-y-4">
                    ...
                    {selectedMode === "auto" && (
                      <div className="pt-2">
                        <button
                          type="button"
                          onClick={() => setShowAutoMultiModal(true)}
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            setSelectedUnitIds(["all"]);
                            setSelectedLessonIds(["all"]);
                            const sub = subjects.find((s) => s.id === newId);
                            if (sub && sub.totalMarks) {
                              setTotalMarks(sub.totalMarks);
                              setCustomSections((prev) =>
        ...
        bottomBarContent={
          <>
            <button className="px-4 py-2 text-xs font-bold text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition">
              حفظ كمسودة
            </button>
      ...
      {showExecutionCenterModal && (
        <AIExecutionCenterModal
          ...
          taskPayload={{
            ...
            previewText: `المادة: ${currentSubject?.name || "عام"} | الوحدات: ${units.map((u) => u.title).join(", ")} | إجمالي الأسئلة: ${totalQuestions}`,
          }}
