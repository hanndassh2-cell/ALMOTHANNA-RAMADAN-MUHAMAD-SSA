...
import { 
  GripVertical, Eye, EyeOff, MoreVertical, ChevronUp, ChevronDown, Lock, Copy, ArrowUp, ArrowDown, Settings, Trash2, Sparkles, X, Edit3 
} from "lucide-react";
...
export const CardShell: React.FC<CardShellProps> = ({
  ...
  return (
    <div
      ...
      className={`lesson-card transition-all duration-150 overflow-visible flex flex-col group cursor-text relative my-1.5 rounded-md ${
        isDragged
          ...
          : isActive
            ...
            : p.customColor
              ...
              : "bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 hover:border-slate-300 dark:hover:border-slate-700 shadow-2xs"
      }`}
    >
      ...
      <div
        className={`px-3 py-1.5 flex items-center justify-between cursor-move transition-all sticky top-0 z-10 rounded-t-md backdrop-blur-md ${
          isActive
            ? "bg-slate-50/90 dark:bg-slate-800/70 border-b border-slate-200/80 dark:border-slate-700/80"
            : "bg-slate-50/60 dark:bg-slate-800/40 border-b border-slate-100/80 dark:border-slate-800/60"
        }`}
      >
        <div className="flex items-center gap-2 w-full min-w-0">
          <div className="p-0.5 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 transition" title="سحب البطاقة">
          ...
          {/* Header Title (Clickable to Expand/Collapse) */}
          <div 
            onClick={(e) => { e.stopPropagation(); toggleCollapse(p.id); }}
            className="flex items-center gap-2 flex-1 min-w-0 cursor-pointer select-none group/title py-0.5"
            title={isCollapsed ? "انقر لتوسيع البطاقة" : "انقر لطي البطاقة"}
          >
            <span className="font-bold text-xs text-slate-700 dark:text-slate-200 group-hover/title:text-blue-600 dark:group-hover/title:text-blue-400 transition-colors truncate">
        ...
        {/* Header Actions */}
        <div className="flex items-center gap-1 shrink-0">
          {/* Explicit "تحرير" (Edit) Button - Always Visible when expanded */}
          {!isCollapsed && (
            <button
              type="button"
              onClick={(e) => {
              ...
              className={`px-2.5 py-1 text-xs font-bold rounded-md transition flex items-center gap-1 cursor-pointer ${
                isActive
                  ...
                  : "bg-blue-50 dark:bg-blue-950/60 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/80 border border-blue-200/80 dark:border-blue-800/80"
              }`}
              title="تحرير محتوى البطاقة"
            >
