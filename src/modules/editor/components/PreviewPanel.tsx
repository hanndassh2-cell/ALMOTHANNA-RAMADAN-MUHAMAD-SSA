...
export const PreviewPanel = ({
  ...
  const previewItems = useMemo(() => {
    ...
    .flatMap((p) => {
      ...
      if (p.type === "notes" && p.body && p.body.startsWith("{")) {
        try {
          ...
          const noteGradients: any = {
            info: "bg-gradient-to-br from-white to-blue-50/50 dark:from-slate-900 dark:to-blue-950/30 border-blue-200 dark:border-blue-900/50 text-blue-900 dark:text-blue-200",
            warning: "bg-gradient-to-br from-white to-amber-50/50 dark:from-slate-900 dark:to-amber-950/30 border-amber-200 dark:border-amber-900/50 text-amber-900 dark:text-amber-200",
            tip: "bg-gradient-to-br from-white to-emerald-50/50 dark:from-slate-900 dark:to-emerald-950/30 border-emerald-200 dark:border-emerald-900/50 text-emerald-900 dark:text-emerald-200",
            quote: "bg-gradient-to-br from-white to-slate-100 dark:from-slate-900 dark:to-slate-800/60 border-slate-300 dark:border-slate-700 text-slate-800 dark:text-slate-200 italic border-r-4",
          };
          const colorClass = p.customColor || (noteGradients[data.type] || `${currentPalette.bgGradient} ${currentPalette.border}`);
          const cardPaddingStyle = activeTemplate.typography?.cardPadding !== undefined
            ? `${activeTemplate.typography.cardPadding}px`
            : "10px 16px";
      ...
      // Map lesson content card types to their standard visual theme colors with alternating pastel gradients
      const getCardColorClasses = (type: string, customColor?: string) => {
        if (customColor) return customColor;
        switch (type) {
          case "title": return "bg-transparent border-none px-0 shadow-none";
          case "objectives": return `border-r-4 border-emerald-500 ${currentPalette.bgGradient} ${currentPalette.border} text-emerald-950 dark:text-emerald-100 rounded-xl`;
          case "concepts": return `border-r-4 border-purple-500 ${currentPalette.bgGradient} ${currentPalette.border} text-purple-950 dark:text-purple-100 rounded-xl`;
          case "explanation": return `${currentPalette.bgGradient} ${currentPalette.border} text-slate-800 dark:text-slate-200 rounded-xl`;
          case "images": return `${currentPalette.bgGradient} ${currentPalette.border} rounded-xl`;
          case "tables": return `${currentPalette.bgGradient} ${currentPalette.border} rounded-xl`;
          case "math": return `${currentPalette.bgGradient} ${currentPalette.border} rounded-xl`;
          case "activities": return `border-r-4 border-orange-500 ${currentPalette.bgGradient} ${currentPalette.border} text-orange-950 dark:text-orange-100 rounded-xl`;
          case "notes": return `border-r-4 border-yellow-400 ${currentPalette.bgGradient} ${currentPalette.border} text-yellow-950 dark:text-yellow-100 rounded-xl`;
          case "examples": return `border-r-4 border-indigo-500 ${currentPalette.bgGradient} ${currentPalette.border} text-indigo-950 dark:text-indigo-100 rounded-xl`;
          default: return `${currentPalette.bgGradient} ${currentPalette.border} text-slate-800 dark:text-slate-200 rounded-xl`;
        }
      };

      const cardColorClass = getCardColorClasses(p.type, p.customColor);
      const isMinimal = p.type === "title";
      const isLargeText = p.type === "explanation" || p.type === "tables";
      const cardPaddingStyle = activeTemplate.typography?.cardPadding !== undefined
        ? `${activeTemplate.typography.cardPadding}px`
        : "10px 16px";

      // Split large text blocks (explanation, concepts, etc) into chunks so PaginatedA4Preview can paginate them properly
      let blocks = [ (p.body || "").replace(/(<p><br><\/p>\s*)+$/g, "").replace(/(<br\s*\/?>\s*)+$/g, "") ];
      
      const canSplit = p.type === "explanation" || p.type === "concepts" || p.type === "objectives" || p.type === "activities" || p.type === "notes" || p.type === "examples";
      if (canSplit) {
  ...
  return (
    <div
      className={`transition-all h-full ${isFullscreen ? "fixed inset-0 z-50 bg-slate-900/95 flex flex-col items-center justify-start overflow-y-auto p-4 sm:p-8 backdrop-blur-sm" : "w-full flex flex-col bg-slate-100 dark:bg-slate-950/50 relative overflow-hidden border-r border-slate-200 dark:border-slate-800"}`}
    >
      <div
        className={`relative z-40 px-3 py-2 border-b border-slate-200 dark:border-slate-800 shrink-0 bg-white dark:bg-slate-900 flex items-center justify-between gap-2 shadow-xs ${isFullscreen ? "w-full max-w-[210mm] mx-auto rounded-t-xl mt-4" : ""}`}
      >
        <div className="flex items-center gap-2">
          ...
          {/* Grouping Engine Quick Control in Preview Panel */}
      ...
      <div className={`flex-1 overflow-auto flex justify-center items-start custom-scrollbar transition-all duration-300 relative z-0 ${
        ...
        <div
          ...
          style={{
            zoom: isNavCollapsed ? Math.max(zoomLevel, 115) / 100 : zoomLevel / 100,
            width: isNavCollapsed ? "66%" : "100%",
            minWidth: isNavCollapsed ? "680px" : undefined,
            maxWidth: isNavCollapsed ? "1400px" : undefined,
          }}
        >
          {/* Live Preview */}
          <PaginatedA4Preview
