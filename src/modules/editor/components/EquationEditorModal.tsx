...
export const EquationEditorModal: React.FC<EquationEditorModalProps> = ({
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-3 sm:p-5 bg-slate-950/65 backdrop-blur-md overflow-hidden animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-5xl max-h-[94vh] sm:max-h-[92vh] flex flex-col overflow-hidden dir-rtl border border-slate-200/80 dark:border-slate-800 my-auto animate-in zoom-in-95 duration-200">
        
        {/* Fixed Header */}
        <div className="px-4 sm:px-5 py-3 border-b border-slate-200 dark:border-slate-800 flex flex-wrap items-center justify-between gap-3 bg-slate-50/95 dark:bg-slate-800/95 backdrop-blur-md shrink-0 select-none sticky top-0 z-20 shadow-sm">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-base font-extrabold text-slate-900 dark:text-slate-100 flex flex-wrap items-center gap-2 leading-tight">
                إدراج معادلة
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-100 dark:bg-blue-900/40 text-[#0f6cbd] dark:text-blue-300">
                  متوافق مع Word وMathLive
                </span>
              </h2>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                ابنِ المعادلة بصريًا ثم أدرجها في موضع المؤشر داخل البطاقة
              </p>
            </div>
          </div>

          <div className="flex items-center gap-3">
             <div className="flex items-center gap-1 rounded-lg border border-slate-200 bg-slate-200/80 p-0.5 dark:border-slate-700 dark:bg-slate-700/80" aria-label="موضع المعادلة">
                <span className="hidden px-2 text-xs font-bold text-slate-500 md:inline dark:text-slate-300">الموضع:</span>
                <button
                  type="button"
        ...
        <div className="bg-slate-100/95 dark:bg-slate-800/95 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 px-3 sm:px-4 py-2.5 flex flex-col gap-2 shrink-0 sticky top-[73px] sm:top-[74px] z-10">
          <div className="flex items-center gap-2">
            <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#0f6cbd] text-xs font-black text-white">1</span>
            <div>
              <div className="text-sm font-extrabold text-slate-800 dark:text-slate-100">اختر البنية أو الرمز</div>
              <div className="text-xs text-slate-500 dark:text-slate-400">ابدأ من النوع المطلوب، ثم املأ الخانات داخل مساحة الكتابة.</div>
            </div>
          </div>
          ...
          {activeCategory && (
            <div className="p-2 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-wrap gap-1.5 animate-in slide-in-from-top-1 duration-150 max-h-[170px] overflow-y-auto">
              {GROUPS.find(g => g.id === activeCategory)?.items.map((item, idx) => (
                <button
                  ...
                  title={item.title}
                  className="flex min-h-14 min-w-[70px] flex-col items-center justify-center rounded-lg border border-transparent px-2 py-1.5 text-slate-800 transition-colors hover:border-blue-200 hover:bg-blue-50 dark:text-slate-200 dark:hover:border-slate-600 dark:hover:bg-slate-700"
                >
                  <span className="mb-1 font-serif text-lg leading-none">{item.icon}</span>
                  <span className="max-w-[76px] truncate text-[11px] font-bold text-slate-500 dark:text-slate-400">{item.label}</span>
                </button>
              ))}
            </div>
          )}
        ...
        <div className="p-4 sm:p-6 bg-white dark:bg-slate-900 flex-1 overflow-y-auto space-y-4 min-h-0">
           <div className="flex items-center justify-between gap-3">
             <div className="flex items-center gap-2">
               <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#0f6cbd] text-xs font-black text-white">2</span>
               <div>
                 <div className="text-sm font-extrabold text-slate-800 dark:text-slate-100">اكتب وعدّل المعادلة</div>
                 <div className="text-xs text-slate-500 dark:text-slate-400">تحرّك بين الخانات بالأسهم أو Tab، ويمكنك استخدام لوحة المفاتيح الرياضية.</div>
               </div>
             </div>
             <span className="hidden rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold text-slate-500 sm:inline dark:bg-slate-800 dark:text-slate-300">اتجاه رياضي LTR تلقائي</span>
           </div>
           {/* Interactive Math Field Canvas */}
           <div className="p-4 sm:p-6 bg-slate-50/80 dark:bg-slate-950/80 rounded-xl border-2 border-slate-200/80 dark:border-slate-800 focus-within:border-[#0f6cbd] dark:focus-within:border-blue-500 transition-colors flex items-center justify-center shadow-inner min-h-[170px]">
              <math-field
                ref={mathfieldRef}
                style={{
                  width: "100%",
           ...
           <div className="border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden bg-slate-50/50 dark:bg-slate-950/50">
             ...
             >
               <span className="flex items-center gap-2">
                 ...
                 لصق من Word أو تحرير الكود (LaTeX / MathML)
               </span>
         ...
         {/* Fixed Footer */}
        <div className="px-4 sm:px-5 py-3 border-t border-slate-200 dark:border-slate-800 bg-slate-50/90 dark:bg-slate-800/70 flex flex-wrap items-center justify-between gap-2 shrink-0 select-none">
          <div className="flex items-center gap-2">
            <button
          ...
          <div className="flex w-full items-center justify-end gap-3 sm:w-auto">
            <span className="hidden items-center gap-1.5 text-xs font-bold text-slate-500 lg:flex dark:text-slate-300">
              <span className="flex h-6 w-6 items-center justify-center rounded-full bg-[#0f6cbd] text-xs font-black text-white">3</span>
              راجع ثم أدرج
            </span>
            <button
              onClick={onClose}
