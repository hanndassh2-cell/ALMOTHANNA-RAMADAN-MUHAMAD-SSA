...
import { createPasswordHashForUser } from "../../../utils/crypto";
...
> = ({
  ...
  const userSavedUi = useMemo(() => storage.getUiState("user_credentials_ui", {
    selectedUserId: activeCurrentUser?.id || usersList[0]?.id || "usr-1",
  }), []);

  const [selectedUserId, setSelectedUserId] = useState<string>(
    userSavedUi.selectedUserId || activeCurrentUser?.id || usersList[0]?.id || "usr-1",
  );
  ...
  const handleCreateNewUser = () => {
    ...
    setToastMessage(
      "تم فتح نموذج إضافة مستخدم جديد. يرجى تعديل البيانات ثم الضغط على (حفظ) للحفظ النهائي.",
    );
    setShowToast(true);
  ...
  const handleResetPasswordAction = async () => {
    const tempPassword = `pass_${Math.floor(100000 + Math.random() * 900000)}`;
