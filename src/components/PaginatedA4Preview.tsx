...
export const PaginatedA4Preview: React.FC<PaginatedA4PreviewProps> = ({
  ...
  useEffect(() => {
    ...
    const runMeasurement = () => {
      ...
      const parentSheet = containerRef.current.closest(".a4-print-sheet");
      const headerEl = parentSheet?.querySelector("header");
      const footerEl = parentSheet?.querySelector("footer");

      const measuredHeaderHeight = (headerEl && headerEl.offsetHeight > 20) ? (headerEl.offsetHeight + 30) : 120;
      const measuredFooterHeight = (footerEl && footerEl.offsetHeight > 15) ? (footerEl.offsetHeight + 24) : 65;
      ...
      children.forEach((child, index) => {
        ...
        let nextItemHeight = nextChild ? nextChild.offsetHeight + gapPx : 0;
    ...
    const timer1 = setTimeout(runMeasurement, 150);
    const timer2 = setTimeout(runMeasurement, 350);
    const timer3 = setTimeout(runMeasurement, 750);

    let resizeObserver: ResizeObserver | null = null;
    if (typeof ResizeObserver !== "undefined" && containerRef.current) {
      resizeObserver = new ResizeObserver(() => {
        if (isSubscribed) runMeasurement();
      });
    ...
    images.forEach((img) => {
      if (!img.complete) {
        img.addEventListener("load", runMeasurement, { once: true });
      }
    });
  ...
  const PageWrapper = ({ children, pageNumber, totalPages, className: additionalClass, isMeasuring: wrapperIsMeasuring }: any) => {
    ...
    const academyName = settings.academyName || "إديوتيك الأكاديمية";
    ...
    return (
      ...
      >
        ...
        <header
          className={`print-document-header border-b-2 border-slate-900 pb-3 mb-6 flex items-start justify-between relative z-10 ${hideHeaderFooter ? "opacity-0 h-0 overflow-hidden mb-0 pb-0 border-0" : ""}`}
      ...
      {measuring && (
        ...
        >
          <PageWrapper pageNumber={1} totalPages={1} isMeasuring={true}>
            ...
            >
              {items.map((item) => (
                <div key={item.id} dir="auto" className="h-auto min-h-0 shrink-0 block">{item.content}</div>
              ))}
            </div>
          </PageWrapper>
        </div>
