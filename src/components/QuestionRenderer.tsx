    ...
    >
      ...
      */}
      ...
      {(qType === "mcq" || qType === "multiple_choice") && distractorsList.length > 0 && (
        ...
        >
          <table
            className="w-full table-fixed border-separate border-spacing-0 border border-slate-700 dark:border-slate-500 print:border-slate-900 text-slate-900 dark:text-slate-100 font-medium"
            style={{
              fontSize: optionsSize,
              fontFamily: activeFontFamily,
              width: "100%",
          ...
          >
            <tbody>
              <tr className="align-middle" style={{ height: "auto" }}>
                {distractorsList.map((d, dIdx) => {
                  ...
                  const isLongText = (d.text || "").length > 40;
                  const colWidth = `${100 / Math.max(1, distractorsList.length)}%`;

                  return (
                    <td
                      key={d.id || `opt-${dIdx}`}
                      className="border border-slate-700 dark:border-slate-500 print:border-slate-900 px-2 py-1 text-right align-middle bg-white/60 dark:bg-slate-800/40 print:bg-transparent overflow-hidden"
                      style={{
                        width: colWidth,
                        maxWidth: colWidth,
                        direction: detectedDir,
                    ...
                    >
                      <div
                        className={`flex items-center justify-start gap-1 w-full min-w-0 max-w-full ${isRTL ? "text-right" : "text-left"}`}
                        dir={detectedDir}
                        style={{
                          fontSize: isLongText ? `calc(${optionsSize} * 0.9)` : optionsSize,
                          fontFamily: activeFontFamily,
                          lineHeight: 1.25,
                        }}
                      >
