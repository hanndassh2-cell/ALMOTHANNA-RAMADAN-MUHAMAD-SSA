...
/**
 * Convert a DOM node from parsed MathML into a LaTeX TeX string
 */

export function convertWordLinearMathToTeX(text: string): string {
  if (!text) return "";

  // Pre-clean input: strip leaked tokens, unescape HTML entities and normalize whitespace
  let cleanInput = cleanLeakedTokens(text)
    .replace(/&amp;\s*amp;/gi, "&")
    .replace(/&amp;\s*/gi, "&")
    .replace(/&nbsp;/gi, " ")
    .replace(/&#160;/g, " ")
  ...
  // Keywords / symbols that indicate a Word linear matrix or array
  const triggers = [
    "■",
    "\u25A0",
    "\u25A1",
  ...
  while (i < cleanInput.length) {
    ...
    // Check for left bracket before the trigger
    let leftBracket = "";
    let leftBracketStart = -1;
    let k = triggerIdx - 1;
    while (k >= i && /\s/.test(cleanInput[k])) k--;

    if (k >= i) {
      if (cleanInput[k] === "[" || cleanInput[k] === "(" || cleanInput[k] === "{" || cleanInput[k] === "|") {
        if (cleanInput[k] === "|" && k > i && cleanInput[k - 1] === "|") {
          leftBracket = "||";
          leftBracketStart = k - 1;
        } else {
          leftBracket = cleanInput[k];
          leftBracketStart = k;
        }
      }
    }

    // Look for '(' following the trigger
    let openParenIdx = triggerIdx + matchedTrigger.length;
    while (openParenIdx < cleanInput.length && /\s/.test(cleanInput[openParenIdx])) openParenIdx++;

    if (openParenIdx >= cleanInput.length || cleanInput[openParenIdx] !== "(") {
      result += cleanInput.slice(i, triggerIdx + matchedTrigger.length);
      i = triggerIdx + matchedTrigger.length;
      continue;
    }

    // Read balanced parentheses starting from openParenIdx
    let depth = 0;
    let closeParenIdx = -1;

    for (let j = openParenIdx; j < cleanInput.length; j++) {
      if (cleanInput[j] === "(") depth++;
    ...
    if (closeParenIdx === -1) {
      result += cleanInput.slice(i, openParenIdx + 1);
      i = openParenIdx + 1;
      continue;
    }

    // Check for right bracket after closeParenIdx
    let rightBracket = "";
    let rightBracketEnd = closeParenIdx + 1;
    let m = closeParenIdx + 1;
    while (m < cleanInput.length && /\s/.test(cleanInput[m])) m++;

    if (m < cleanInput.length) {
      if (cleanInput[m] === "]" || cleanInput[m] === ")" || cleanInput[m] === "}" || cleanInput[m] === "|") {
        if (cleanInput[m] === "|" && m + 1 < cleanInput.length && cleanInput[m + 1] === "|") {
          rightBracket = "||";
          rightBracketEnd = m + 2;
        } else {
          rightBracket = cleanInput[m];
          rightBracketEnd = m + 1;
        }
      }
    }
    ...
    // Strip inner HTML tags if present
    innerContent = innerContent.replace(/<[^>]*>/g, "");
    // Clean any remaining amp; or &amp; inside innerContent
    ...
    // Clean space around variable names or equal signs in prefix e.g. "A=" -> "A = "
  ...
  // Convert Word integrals / sums / products / limits
  result = result
  ...
  // Clean any residual &amp; or amp; from output matrix TeX
  result = result
