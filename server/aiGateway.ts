...
// Check for forbidden raw keys in client request
export function sanitizeClientRequest(body: any) {
  if (body.apiKey && typeof body.apiKey === "string" && body.apiKey.trim() !== "" && !body.apiKey.startsWith("••••")) {
    throw new Error("Raw browser API keys are strictly rejected. Please configure credentials in the secure Server Vault.");
  }
}
